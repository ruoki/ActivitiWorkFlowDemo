...
import com.zr.activiti.utils.RevokeTaskCmd;
...
public class CusTaskService{
	...
	public List<BaseVO> findTodoTask(User user, Page<BaseVO> page) {
		TaskQuery taskQuery = this.taskService.createTaskQuery().taskCandidateOrAssigned(user.getUserId()).orderByTaskCreateTime().desc();
...
//			tasks = taskQuery.listPage(pageParams[0], pageParams[1]);
	...
	public List<User> excuteFirstTask(String instanceId, String paramsJson,BaseVO baseVO, Map<String, Object> variables) throws Exception {
		Task task = taskService.createTaskQuery()// 查询出本流程实例中当前仅有的一个任务“提交申请”
				.processInstanceId(instanceId).singleResult();
//		baseVO.setHandledTaskId(task.getId());
//		baseVO.setAssignedId(baseVO.getCreateId());
//		baseVO.setAssignedName(baseVO.getCreateName());
		baseVO.setTaskDefinitionKey(task.getTaskDefinitionKey());
		StringBuilder handleFlag = new StringBuilder();
		checkIsPassFirstTask(paramsJson, baseVO, variables, handleFlag);
...
//			variables.put("autoComplete", true);//如果下一节点执行人为空，则直接通过
		...
		List<User> userList = handleTask(task.getId(),baseVO.getCreateId(),handleFlag, "发起申请", baseVO, variables);
		return userList;
	}
	 ...
	 * @param handleFlag
	 */
	private String checkIsPassFirstTask(String paramsJson, BaseVO baseVO,
			Map<String, Object> variables, StringBuilder handleFlag) {
		String pass = "";
		String isPassStr = JsonUtil.get().getProperty(paramsJson, "isPass");
	 ...
	 * @return 
	 */
	public List<User> handleTask(String taskId,String userId,StringBuilder handleFlag, String content, BaseVO baseVO,
			Map<String, Object> variables) {
//		final String taskId = baseVO.getHandledTaskId();
		...
		List<User> userList = getNextNodeAssigneInfos(baseVO.getProcessInstanceId());
		return userList;
	}
	 ...
	 * @return
	 */
	private List<User> getNextNodeAssigneInfos(String processInstanceId) {
		...
		if (null != toDotaskList && toDotaskList.size() > 0) {
			...
			for (int i = 0; i < userIds.length; i++) {
				User user = this.userService.findUserById(userIds[i]);
				if (user != null) {
					userList.add(user);
				}
			}
	...
	public List<BaseVO> findDoneTask(User user, Page<BaseVO> page) throws Exception {
...
//			hTaskAssigneeList = historyTaskQuery.listPage(pageParams[0], pageParams[1]);
	...
	public String getUserName(String userId){
		...
		if (userId.contains(",")) {
			...
			for (int i = 0; i < userIds.length; i++) {
				userName += getUserNameFromDB(userIds[i]) + ",";
			}
		} else {
	...
	private String getUserNameFromDB(String userId) {
		String userName = "";
		// List<User> userList = this.userService.findByCode(userCode);
